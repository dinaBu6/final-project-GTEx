...
#---------Third OPTION: Quantile Normalization + Reggresion---------
...
normBrainData <- as.data.frame(quantileNormalization(brainData))


#plot boxplot before and after normalization
par(mfrow = c(1,2))
boxplot(brainData[,2:12], names = NULL, main = "Examples of genes before\n quantile normalization")
...
#calculate the PCA for the narmolized data 
pca <- prcomp(t(normBrainData), scale=TRUE) 
...
orderedBrainData <- normBrainData[order(names(normBrainData))]
...
#plot the regression with regression line
plotRin <- function(i) {
...
#plot every one of the random genes
...
write.csv(FixedBrainData, "QuantRegBrainData.csv")
